...
namespace Datadog.Trace.ClrProfiler.AutoInstrumentation.Azure.ServiceBus
{
    ...
    public class TransportSenderSendAsyncIntegration
    {
            ...
            where TTarget : ITransportSender, IDuckType
        {
            ...
            {
                // TODO: We may be able to remove this map lookup by making this integration work only on the
                // type Azure.Messaging.ServiceBus.Amqp.AmqpSender instead of the TransportSender abstract class.
                // Then we could access the _connectionScope.ServiceEndpoint.Host
                if (TransportSenderHelper.TryGetFullyQualifiedNamespace(instance.Instance, out var fullyQualifiedNamespace))
                {
                    span.SetTag("out.host", fullyQualifiedNamespace);
                    span.SetTag("peer.service", fullyQualifiedNamespace);
                    span.SetTag("_dd.peer.service.source", "network.destination.name");
                }
